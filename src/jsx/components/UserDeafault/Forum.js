...
function Forum() {
  ...
  useEffect(() => {
    ...
    getPostData().then((response) => {
      let listPostsNeed = response.data.filter((post) => {
        return (
          post.accept === true && (post.baned === false || post.baned === null)
        );
      });
      setListPosts(listPostsNeed);
      setListPostsView(getPostByQuality(0, 3, listPostsNeed));
  ...
  const fetchMoreData = () => {
    ...
    setListPostsView([
      ...listPostsView,
      ...getPostByQuality(0, listPosts.length, listPosts),
    ]);
  ...
  const handleSubmitComment = (postId, userId, comment) => {
    ...
    try {
      ...
      const response = axios
        .post("https://6618-1-53-195-148.ngrok-free.app/api/Comments", data)
        .then((response) => {
                          ...
                          {post.content.length < 100 ? (
                            <>
                              ...
                              <a
                                href="#"
                                onClick={(e) => {
                                  handleShowDetail(post.postId, e);
                                }}
                              >
                                <h5>
